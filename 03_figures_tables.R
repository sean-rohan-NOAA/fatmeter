...
ftnir_data <- 
  ...
  dplyr::select(YEAR, SPECIMEN_NUMBER, common_name, name_abbv, CRUISE, VESSEL, HAUL, SEX, AGE, LENGTH_CM, TOTAL_WT_G, LIVLIPID, Lipid_pred_LOOCV, Lipid_pred_cal, P_LIVLIPID) |>
  dplyr::mutate(
    category = "FT-NIR",
    error = Lipid_pred_LOOCV/100-P_LIVLIPID,
    rel_error = error/P_LIVLIPID,
    abs_error = abs(Lipid_pred_LOOCV/100-P_LIVLIPID), 
    model_name = "PLSR"
  )
...
ftnir_metrics <- 
  ...
  dplyr::summarise(
    ...
    mre = mean(abs_error/P_LIVLIPID),
    mae = mean(abs_error),
    r2 = cor(P_LIVLIPID, Lipid_pred_LOOCV)^2,
...
obs_pred <- 
  ...
  dplyr::bind_rows(
    ftnir_data |>
      dplyr::mutate(fit = Lipid_pred_cal/100) |>
      dplyr::select(common_name, name_abbv, YEAR, LIVLIPID, fit, category)
  )
