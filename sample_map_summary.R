...
dat <- 
  dplyr::bind_rows(
    readxl::read_xlsx(path = "./data/fatmeter_data_dec_2025.xlsx", sheet = "WP") |>
      dplyr::mutate(`Specimen Number` = as.character(`Specimen Number`)),
    readxl::read_xlsx(path = "./data/fatmeter_data_dec_2025.xlsx", sheet = "PCOD")
  ) %>%
  rename_with(~ .x %>%
  ...
  dplyr::mutate(
    COMMON_NAME = ifelse(SPECIES == "WP", "walleye pollock", "Pacific cod"),
